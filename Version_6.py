...
def loop_game():#function for looping game
      while True:#while true
          easygui.msgbox("Do you want to play again? (answer on next slide)")#asks the user if they want to play again
          play_again = easygui.buttonbox("Choose one:", choices=["Yes", "No"])#choices for this question
          if play_again == "No":#if they press no then
              exit()#stop the code
          elif play_again == "yes":#if they press yes then
            global current_points
            current_points = 0#points are set to 0
